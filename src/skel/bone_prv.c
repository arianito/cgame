...
void bone_upd_transform(Skel *self, int bone)
{
    ...
    if (it->parent != -1)
    {
        ...
        it->local_rotation = clamp_axisf(it->world_rotation0 + atan2df(m2.m[1][0], m2.m[0][0]));
...
void update_matrices(Skel *self, bool update_world)
{
    ...
    for (int i = from_index; i < skel->bones->length; i++)
    {
        ...
        if (it->parent == -1)
        {
            ...
            if (update_world)
            {
                it->world_position0 = it->local_position;
                it->world_rotation0 = it->local_rotation;
                it->world_scale0 = it->local_scale;
        ...
        if (update_world)
        {
            it->world_position0 = mat3_mulv2(pt->world, it->local_position, 1);
            Mat3 m2 = pt->world;
            it->world_rotation0 = clamp_axisf(it->local_rotation + atan2df(m2.m[1][0], m2.m[0][0]));

            float det = m2.m[0][0] * m2.m[1][1] - m2.m[0][1] * m2.m[1][0];
            it->world_scale0.x = sqrf(m2.m[0][0] * m2.m[0][0] + m2.m[1][0] * m2.m[1][0]);
            it->world_scale0.y = it->local_scale.y * (det / it->world_scale0.x);
            it->world_scale0.x *= it->local_scale.x;
