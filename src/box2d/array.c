...
{
	void* result = (b2ArrayHeader*)xxmalloc(sizeof(b2ArrayHeader) + elementSize * capacity) + 1;
...
void b2Array_Grow(void** a, int32_t elementSize)
{
	...
	void* tmp = *a;
	*a = (b2ArrayHeader*)xxmalloc(sizeof(b2ArrayHeader) + elementSize * newCapacity) + 1;
