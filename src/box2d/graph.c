...
void b2CreateGraph(b2Graph* graph, int32_t bodyCapacity, int32_t contactCapacity, int32_t jointCapacity)
{
	*graph = (b2Graph){0};
	...
	bodyCapacity = maxf(bodyCapacity, 8);
	contactCapacity = maxf(contactCapacity, 8);
	jointCapacity = maxf(jointCapacity, 8);
...
static inline int32_t GetWorkerStartIndex(int32_t workerIndex, int32_t blockCount, int32_t workerCount)
{
	...
	return blocksPerWorker * workerIndex + minf(remainder, workerIndex);
}
...
static bool b2SolveGraph(b2World* world, b2StepContext* stepContext)
{
	...
	*/
	...
	int32_t velIters = maxf(1, stepContext->velocityIterations);
